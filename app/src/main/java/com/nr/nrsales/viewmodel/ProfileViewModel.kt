...
class ProfileViewModel @Inject constructor
    ...
    val userDashboardModel: MutableLiveData<NetworkResult<UserDashboardModel>> = MutableLiveData()

    fun fetchupdateUserResponse(body: RequestBody) = viewModelScope.launch {
        repository.updateUser(body).collect { values ->
    ...
    fun fetchUserDashboard(params: HashMap<String, Any>) = viewModelScope.launch {
        repository.user_dashboard(params).collect { values ->
            userDashboardModel.value = values
        }
    }
}
